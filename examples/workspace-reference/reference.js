...
const copy = {
  zh: {
    ...
    contrast: "切换高对比度", moreActions: "更多操作", copyLink: "复制记录链接", copiedLink: "记录链接已复制", copyLinkFailed: "无法复制记录链接",
    entityViews: "记录视图", summaryTab: "概览", activityTab: "事件",
    sourcesTab: "来源", relationsTab: "关联", description: "持续记录入口、公开频道、运行状态和可验证事件。",
    follow: "加入关注", runScan: "运行检测", metrics: "关键指标", currentStatus: "当前状态",
    healthy: "正常", operatingRecord: "运营时长", operatingValue: "6 年 3 个月", completeness: "数据完整度",
    ...
    currentObject: "当前记录", closeInspector: "关闭属性面板", status: "状态", operatingStatus: "运行状态",
    ...
    confidence: "证据可信度", confidenceDefinition: "该分数表示当前证据来源的可信程度，不代表记录整体安全等级。",
    ...
    investigations: "调查记录",
    riskSavedView: "高优先级待复核", riskReview: "复核", riskQueueViews: "风险队列视图", riskScopeElevated: "高优先级", riskScopeUnassigned: "未指派", riskScopeConflicting: "证据冲突", riskScopeResolved: "已解决",
    riskSavedViewLabel: "已保存视图 · 过去 24 小时", riskQueueTitle: "高优先级待复核", riskQueueDescription: "先核对严重度与证据状态，再指派负责人或记录可撤销结论。", riskRefresh: "刷新当前范围", riskFixtureNote: "固定的本地示例数据，基准时间为 2026-07-19 10:32 +08:00；相对时间均以该时刻计算。未连接实时监测、权限服务或持久化存储。",
    riskScopeSummary: "当前范围摘要", riskScopeLabel: "范围", riskScopeValue: "高或严重 · 未解决", riskSortLabel: "排序", riskSortValue: "严重度 → 观察时间", riskCoverageLabel: "证据要求", riskCoverageValue: "至少 2 个已标明出处的来源",
    riskLoadingTitle: "正在刷新当前范围", riskLoadingBody: "正在刷新本地示例数据；当前队列与选择保持不变。", riskConflictTitle: "证据存在冲突", riskConflictBody: "两个来源对证书变更是否预期给出不同结论，需记录人工判断。", riskPendingTitle: "正在记录复核决定", riskPendingBody: "记录期间暂时无法编辑；队列范围与草稿保持不变。",
    riskCommittedTitle: "复核决定已记录", riskCommittedBody: "该决定仍可撤销；风险等级本身未被自动改写。", riskUndoTitle: "复核决定已撤销", riskUndoBody: "信号回到待复核状态，原理由保留在本地草稿中。", riskPermissionTitle: "仅可查看", riskPermissionBody: "此示例展示无复核权限的状态；队列与证据仍可读取。", riskErrorTitle: "无法记录复核决定", riskErrorBody: "本地草稿、负责人和理由均已保留，可以重试。",
    riskTableTitle: "信号队列", riskTableDescription: "严重度、证据状态和复核状态分别显示，不合并为单一分数。", riskCount: "4 个本地信号", riskCountEmpty: "0 个本地信号", riskEmptyTitle: "当前范围没有待复核信号", riskEmptyBody: "可以切换已保存视图；本地示例不会自动扩大范围。", riskTableLabel: "风险信号队列",
    riskColumnSignal: "信号", riskColumnSeverity: "严重度", riskColumnEvidence: "证据状态", riskColumnOwner: "负责人", riskColumnReview: "复核状态", riskColumnObserved: "观察时间",
    riskSignal204: "证书签发者发生变化", riskSignal198: "公开频道入口不可用", riskSignal191: "新增关联域名待核验", riskSignal184: "节点延迟已恢复", riskSeverityCritical: "严重", riskSeverityHigh: "高", riskSeverityMedium: "中", riskSeverityLow: "低", riskEvidenceConflict: "冲突", riskEvidenceConfirmed: "已确认", riskEvidencePending: "待核验", riskUnassigned: "未指派", riskReviewOpen: "待复核", riskReviewActive: "复核中", riskReviewEvidence: "等待证据", riskReviewResolved: "已解决", riskObservedEight: "8 分钟前", riskObservedFortyOne: "41 分钟前", riskObservedTwoHours: "2 小时前", riskObservedThreeHours: "3 小时前",
    riskSelectedSignal: "选中信号", riskSignalContext: "信号上下文", riskEvidenceRows: "附有可追溯来源的证据", riskReviewDecision: "复核决定", riskOwnerLabel: "负责人", riskChooseOwner: "选择负责人", riskDecisionLabel: "决定", riskDecisionWatch: "保持观察", riskDecisionEscalate: "升级复核", riskDecisionResolve: "确认为预期变更并关闭", riskReasonLabel: "记录理由", riskReasonPlaceholder: "说明证据如何支持该决定", riskRecordDecision: "记录决定", riskRecordingDecision: "正在记录", riskRecordedTitle: "已记录的决定", riskRecordedAt: "记录时间", riskReviewSignal: "复核选中信号",
