...
| File | Use |
|---|---|
| [`kin.tailwind.css`](./kin.tailwind.css) | Tailwind CSS v4 `@theme` variables plus normative material and motion custom properties |
...
The Figma payload creates `KIN Color`, `KIN Scale`, `KIN Typography`, `KIN Material`, and `KIN Motion`. Color and Material include Light, Dark, Light High Contrast, and Dark High Contrast modes. Typography composites are flattened into the primitive STRING and FLOAT types supported by Figma Variables. Material edge values are COLOR variables. Material shadow recipes are exported as CSS-compatible STRING variables for handoff; this repository does not claim that they are published Figma Effect Styles. Motion durations are exported as millisecond FLOAT values and easing curves as CSS-compatible STRING values because Figma Variables do not expose composite duration or cubic-bezier types.
