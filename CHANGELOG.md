...
- Reframed the public Showcase as an instrument gallery rather than a documentation catalog: the Home first view now pairs a short product statement with a dominant dark live workspace and vertical Scenario rail, product contexts use an editorial reading layout, workflow continuity uses a spatial four-step trace, Component and Pattern discovery place compact navigation beside the runnable stage, and Component Explorers surface canonical-reference language mismatches in the first view. Governance and catalog evidence remain available below the product proof.
...
- Promoted Button to a first-class Component Explorer and replaced its repeated variant matrix with one task-led interaction lab. The focused reference now demonstrates action hierarchy, paired preview, stable-width async success and retry, visible Sonner result notifications, a keyboard-recoverable Overflow Menu, an accessible Icon Button Tooltip, density controls, and destructive confirmation with undo without changing Button maturity or claiming product-side request behavior.
- Completed Simplified Chinese continuity across Showcase navigation, featured Component names, Component Explorers, Pattern evidence, Scenario Atlas, Scenario Lab, and page-pattern references. Chinese routes now preserve locale through shared navigation and inspection URLs, load Chinese-capable references with `lang=zh-CN`, and use page-specific copy keys instead of substring replacement. User-facing workflows distinguish identity verification, human review, release approval, input validation, and evidence verification; implementation terms such as reference boundaries, adopting products, and deterministic fixtures are kept out of task copy while canonical IDs and English source contracts remain unchanged.
