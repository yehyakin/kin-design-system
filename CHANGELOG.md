...
- Added the experimental KIN Design Engineer Codex plugin: a runnable audit, authorized-repair, and verification workflow that uses the active Codex model, reuses the authoritative KIN contracts, collects deterministic read-only candidates, and keeps GPT-5.6 identity, rendered evidence, accessibility claims, and the proposed Agent Distribution Layer explicitly bounded.
- Continued Scenario Atlas Phase 3 by promoting five stable shared workflows plus intelligence risk review, information reading, revision history, commerce product editing, and engineering hierarchy scenarios; the Atlas now exposes 16 showcased scenarios and 47 independently checked local states while 14 entries remain visibly planned. Risk review keeps saved scope, severity, evidence state, owner, and review status separate through loading, empty, conflict, pending, committed, undo, permission, and failed-record fixtures; Commerce editing preserves product, money, inventory, channel, approval, and activity context. Both remain deterministic local references without live monitoring, backend persistence, or publication claims.
