...
8. 接入 KIN 时显式选择主产品类型，为混合产品按路由映射类型，并完成 [`adoption/implementation-brief.md`](./adoption/implementation-brief.md) 定义的项目实施简报；禁止默认把所有路由当作同一种页面。

实现完整页面或跨页流程时，Agent MUST 先查阅 [`principles/visual-signature.md`](./principles/visual-signature.md)、[`pages/catalog.md`](./pages/catalog.md) 与对应 `pages/` 合同，确认该页面族的视觉签名、成熟度、已知缺口和证据边界。组件齐全不代表页面流程已经完整，Token 映射与组件实验页也不代表产品已经呈现 KIN。

若项目存在 `kin.config.json`，Agent MUST 在修改范围页面前读取其中的 `scope.implementationBrief` 和 `scope.routeProfiles`。实施简报仍为 `draft`、含有 `TODO`、没有唯一代表性工作流，或把组件画廊当作生产流程时，MUST 停止页面实现并先解决这些决策。
...
- 核心工作台只使用一套单色线性图标。
- KIN 参考实现使用 [Lucide](https://lucide.dev/)；已有产品 MAY 保留另一套完整、合法且视觉一致的线性图标库，但 MUST NOT 与 Lucide 混用。
- 必须按需导入实际使用的图标，禁止把整套图标数据打入客户端包。
...
- 仅图标控件必须有 accessible name；含义不普遍时还必须有 Tooltip。装饰性图标应从辅助技术中隐藏。
...
├── Context Sidecar（可选）：与当前任务并行使用的证据、队列或工具上下文
...
- `768–1023px`：Sidebar 折叠，Inspector 与 Context Sidecar 使用互斥 overlay。
- `< 768px`：单栏；底部导航或顶层返回；属性与任务上下文进入 Drawer、Sheet 或详情页。
...
组件的规范名称与非同义词见 [`components/terminology.md`](./components/terminology.md)，成熟度、合同、参考与测试状态见 [`components/catalog.md`](./components/catalog.md)。适用组件的完整状态、交互与验收矩阵见 [`components/core-states.md`](./components/core-states.md)。Location Bar、Toolbar 与 Split View 的结构和交互合同见 [`components/workspace-structure.md`](./components/workspace-structure.md)。操作与选择、表单与输入、导航与 Disclosure、数据展示、反馈与进度、浮层分别由 [`actions-and-selection.md`](./components/actions-and-selection.md)、[`forms-and-entry.md`](./components/forms-and-entry.md)、[`navigation-and-disclosure.md`](./components/navigation-and-disclosure.md)、[`data-display.md`](./components/data-display.md)、[`feedback-and-progress.md`](./components/feedback-and-progress.md) 和 [`overlays.md`](./components/overlays.md) 定义。小型状态变化、异步结果、图标替换与 Reduced Motion 的完整合同见 [`components/micro-interactions.md`](./components/micro-interactions.md)。这些文件均为规范性要求。

Agent MUST NOT 把 `draft` 或 `candidate` 组件描述为完整 KIN 组件。组件外观相似不代表行为等价；Tooltip、Popover、Dropdown Menu、Select 与 Combobox 等术语 MUST 按术语合同区分。
...
- 折叠必须保留当前位置、活动项、导航顺序和可访问名称；不得把折叠后的 Sidebar 变成一组无法解释的图标。
- 折叠时图标保持稳定光学位置，文字在受控容器中淡出或裁切；不得让每一项横向滑动、弹跳或重新挂载。
- 折叠态的陌生图标必须在 hover 与 focus 时显示 Tooltip，并为触控与小屏提供非 hover 路径。
- 折叠与展开必须可中断、可反向，并遵循 panel duration；Reduced Motion 使用即时布局切换或短 crossfade。
- 持久化折叠偏好时必须按产品与布局版本命名，并在移动端替代布局中忽略不适用的桌面值。
...
Context Sidecar 是与主任务并行存在的辅助工作区域。它可承载证据、排期上下文、审阅队列、帮助或真实 AI 工具，但不是第二个 Dashboard，也不是默认常驻聊天栏。

- 只有在用户需要持续交叉查看主任务与辅助上下文时使用；短暂内容使用 Popover 或 Drawer，稳定对象属性使用 Inspector。
- 宽屏 MAY 通过相邻轨道重排 Workspace；只有当主任务和 Sidecar 都保持各自最小可用宽度时才允许重排。
- 窄屏 MUST 适配为可关闭的 Drawer、Sheet 或独立详情；不得仅把 Workspace 挤到不可用宽度。
- 非模态宽屏 Sidecar 不得截留焦点；模态窄屏适配必须遵循 Dialog/Drawer 的焦点、背景与返回规则。
- 打开、关闭和快速反向必须保留主任务的选择、滚动、输入与历史状态。
- AI 内容只有在真实后端、来源、停止、失败与恢复边界存在时才能显示为 AI 能力；本地参考必须明确标注 fixture。

完整合同见 [`components/workspace-structure.md`](./components/workspace-structure.md)。
...
身份验证、访问恢复、验证码和会话重新验证的完整边界见 [`components/authentication.md`](./components/authentication.md) 与 [`pages/authentication-and-access.md`](./pages/authentication-and-access.md)。界面参考 MUST NOT 被描述为真实认证实现；凭据处理、限流、枚举防护、会话和服务端授权仍由接入产品负责。
...
外观与语言属于全局偏好，不是装饰性工具栏按钮。完整结构、状态与验收要求见 [`components/preference-controls.md`](./components/preference-controls.md)。

- 外观偏好 MUST 支持 `light`、`dark` 与 `system`；紧凑开关 MAY 直接切换当前解析出的明暗主题，但 MUST 保留恢复 `system` 的入口。
- KIN 参考开关使用 Sun、Track、Moon 的紧凑结构；控件 MUST 使用 `role="switch"`、正确的 `aria-checked` 和描述下一动作的 accessible name。
- 语言入口 SHOULD 使用统一图标系统中的语言图标；菜单 MUST 用 `中文`、`English` 等语言自身名称展示选项，并明确当前语言。
- 只有完整、经过审核的翻译可用时才显示对应语言；禁止只切换图标、`lang` 属性或少量标题来伪装多语言支持。
- 切换主题或语言 MUST NOT 清空输入、重建应用、丢失当前对象、重启任务或显示成功 Toast。
...
- 条件页面（例如计费）只在真实产品能力存在时采用，MUST NOT 为了填满导航而创建。

页面目录、成熟度和完成定义见 [`pages/catalog.md`](./pages/catalog.md)，机器可读记录见 [`pages/catalog.json`](./pages/catalog.json)。
...
动效不得把 Hover 或 Focus 表达成已完成操作。复制、保存、上传、提交等成功状态 MUST 在真实操作成功后出现；状态切换、异步结果、Disclosure 与偏好变化的触发和回退规则见 [`components/micro-interactions.md`](./components/micro-interactions.md)。

### 9.2 频率与输入方式

动效强度 MUST 由任务频率和调用路径共同决定，而不是由组件名称决定：

| 频率或调用路径 | KIN 默认 |
|---|---|
| 键盘优先，或预计每日出现 100 次以上 | 使用 `--duration-instant`；不做空间入场；焦点和输入必须在同一交互任务中可用 |
| 预计每日出现数十次 | 不做位移、缩放或装饰性重播；必要的颜色或透明度反馈不超过 `--duration-fast` |
| 偶尔出现的 Dialog、Drawer、Inspector 或 Toast | 可使用 normal / panel 时长解释空间、状态或结果 |
| 罕见、首次或解释性流程 | MAY 使用更完整的序列，但不得阻止输入、隐藏结果或建立新的产品默认 |

- 同一组件 MAY 根据键盘、指针或手势来源降低动效，但 MUST 保持相同语义、焦点结果和最终状态。
- 键盘触发不等于删除反馈；Focus、Selection、Pressed、Busy 和 Committed Result 仍 MUST 可见。
- Command Menu 的键盘路径 MUST 即时打开并聚焦搜索；不得等待入场、缩放或位移动画。
- 频率判断、调用来源和减弱方案 MUST 在组件或产品实施记录中可复核。

操作术语使用 [`principles/motion-vocabulary.md`](./principles/motion-vocabulary.md)；直接操控、速度与边界行为使用 [`principles/apple-interaction.md`](./principles/apple-interaction.md)。
...
进入与退出都 MUST 在第一批可见帧中响应。退出使用更短的时长预算和 `--ease-exit`，不得使用慢启动的 ease-in 曲线延迟关闭反馈。
...
标准 Chart、键盘检查、数据表 fallback、实时更新与分析语义的完整合同见 [`components/charts-and-analysis.md`](./components/charts-and-analysis.md)。
...
AI Composer、Streaming Response、Evidence List、停止与重试的完整合同见 [`components/ai-assistance.md`](./components/ai-assistance.md)。Suggested Change Review、Execution Preview、Media Review、批准与回滚见 [`components/review-and-approval.md`](./components/review-and-approval.md)。后台任务、队列、取消、部分完成与持久状态见 [`components/background-work.md`](./components/background-work.md)。
...
第三方库提供成熟行为，不提供最终产品语义与视觉。KIN SHOULD 直接运行经过评审的官方包，不得为了“统一”而重写其动画、虚拟化、拖放、输入或图表引擎。KIN 负责组件语义、适用边界、Token、主题、响应式组合、可访问性补充、测试和回滚。

### 12.0 上游保真契约

- 已评审能力 MUST 通过官方发布包直接集成；“直接集成”不等于把第三方源码复制进 KIN。
- 上游拥有的手势、插值、弹簧、缓动、时序、堆叠、焦点、测量与输入行为 MUST 保持原实现。KIN MUST NOT 用相似的 CSS 或自制状态机重现它们。
- 当上游视觉已符合 KIN 时，适配层 SHOULD 保留原样；需要适配时 SHOULD 优先使用官方 class、property、theme API、CSS Custom Property 或组合层，而不是 fork 内部实现。
- KIN MAY 限制不适合产品语境的可选效果，例如装饰粒子、发光或首次加载计数；这类限制 MUST 不改变组件的核心任务反馈。
- 只有存在已复现的可访问性、安全、平台兼容或产品语义冲突时，KIN MAY 改变上游行为。修改 MUST 记录原因、测试、负责人、迁移影响、回滚和移除条件。
- 上游已经支持 `prefers-reduced-motion` 时，KIN MUST 使用并验证该路径，不得用另一个业务状态（例如 paused、loading 或 disabled）冒充 Reduced Motion。
- 每个运行时集成 MUST 在 [`integrations/catalog.json`](./integrations/catalog.json) 分别登记 `preserved_upstream` 与 `kin_owns`，并用正常动效和 Reduced Motion 证据证明边界。

### Core visual adapter

- `Lucide`：KIN 参考实现的单色线性图标库。按需导入、使用 `currentColor`，默认 1.5px stroke，不与第二套 UI 图标混用。

### Runtime-integrated stable contracts

- `cmdk`：全局命令与搜索。
- `Virtuoso`：真实大数据长列表与持续事件流。
- `Sonner`：用户主动操作反馈。

### Runtime-integrated candidate contracts
...
这些运行时适配位于私有预发布 [`packages/react/`](./packages/react/) 中；`runtime-integrated` 只表示官方依赖、KIN API、可操作示例与自动化证据同时存在，不会自动提升 [`components/catalog.md`](./components/catalog.md) 的组件成熟度。完整登记见 [`integrations/catalog.md`](./integrations/catalog.md)。
...
验证证据 MUST 遵循 [`principles/verification.md`](./principles/verification.md)：自动化检查、截图复核、真实浏览器缩放和读屏人工检查必须分别记录。不得用自动化测试或截图声称完整 WCAG 符合性，也不得把 Firefox/WebKit 冒烟测试描述成完整浏览器一致性。
...
10. Quality audit：视觉、可访问性、性能、Anti-Slop。
11. Removal：新版本功能对等且稳定后再删除旧实现。
...
candidate screenshots with realistic content at named viewports, and record a
human visual-signature review before claiming the workflow is verified.
```
...
Agent 接入、审计脚本和证据格式属于 KIN 的采用能力，不是 KIN 的产品方向。KIN 的完整方向由 [`VISION.md`](./VISION.md) 定义。
