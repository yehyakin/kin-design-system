...
Implementation record (2026-07-19): catalog version 1.5.0 promotes INT-03 without changing the stable Intelligence Workspace source maturity. The existing workspace reference now exposes a saved-scope risk queue with a dominant comparable signal list and a selected-signal Inspector. Severity, evidence state, owner, and review status remain separate through normal, loading, empty, conflict, pending, locally committed, undo, permission, and failed-record routes. Assignment, recorded reason, retry, and undo are deterministic local interactions; no live monitor, permission service, durable review record, or production adoption is claimed. The Atlas now contains 16 showcased scenarios, 14 planned entries, and 47 deterministic state routes; investigation, monitor recovery, order and campaign operations, scoped batch changes, generated-change review, conflict handling, and durable background operations remain planned.

Implementation record (2026-07-21): catalog version 1.6.0 promotes INT-02 without changing the stable Intelligence Workspace source maturity. The workspace reference now exposes an entity-led investigation route whose dominant region is a flat chronology rather than a queue, dashboard, or Card wall. Occurrence time, observation time, source identity, source availability, verification state, human finding, and supporting-evidence attribution remain separate through normal, loading, partial, conflict, stale, empty, pending, locally committed, undo, permission, and failed-record routes. Scope filtering, selected-event URL state, draft preservation, retry, undo, and the narrow evidence Drawer are deterministic local interactions; no live source, authoritative permission, durable finding record, or production adoption is claimed. The Atlas now contains 17 showcased scenarios, 13 planned entries, and 58 deterministic state routes; monitor recovery, order and campaign operations, scoped batch changes, generated-change review, conflict handling, and durable background operations remain planned.

Verification record (2026-07-21): all required documentation, design, component, page, scenario, integration, release, Token, and Figma Variables checks passed; `npm run test:tooling` passed 23 of 23 tests; `npm run runtime:check` and `npm run site:check` passed; and `npm run test:reference -- --workers=4` passed 130 of 130 tests across Chromium Reduced Motion, Chromium normal motion, Firefox, and WebKit. Focused automated evidence covers chronology and source semantics, scope and selected-event history, validation focus, pending, committed, undo, partial, stale, empty, permission, failed-record retry, preserved local drafts, narrow Drawer containment and focus restoration, minimum touch targets, localization, high contrast, Reduced Motion, cross-browser dynamic-module loading, and horizontal overflow. Visual review covered the wide dark chronology and Inspector, the narrow light high-contrast evidence Drawer, and the narrow Atlas failed-finding fixture. Manual screen-reader, browser-zoom, physical touch-device, live-source, durable-persistence, and consuming-product evidence remain outside this record.

Verification record (2026-07-19): all required documentation, design, component, page, scenario, integration, release, Token, and Figma Variables checks passed; `npm run test:tooling` passed 23 of 23 tests; `npm run runtime:check` and `npm run site:check` passed; and `npm run test:reference` passed 128 of 128 tests across Chromium Reduced Motion, Chromium normal motion, Firefox, and WebKit. Focused automated review covered saved-scope history, separate severity, evidence, owner, and review semantics, validation focus, pending, committed, undo, permission, empty, failed-record retry, preserved local drafts, narrow Drawer focus restoration, minimum touch targets, Reduced Motion, localization, and horizontal overflow. Visual review covered the wide dark queue plus narrow light and light high-contrast Drawer captures. Manual screen-reader, browser-zoom, physical touch-device, live-service, durable-persistence, and consuming-product evidence remain outside this record.
