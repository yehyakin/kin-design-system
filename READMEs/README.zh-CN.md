...
当前合同为开发中的 KIN 3.0.0。需要不可变的生产接入版本时，请在 3.0.0 正式发布前使用[最新稳定版 v2.3.0](https://github.com/yehyakin/kin-design-system/releases/tag/v2.3.0)。
...
[`VISION.md`](../VISION.md) 说明 KIN 的产品方向，以及设计系统、交付表面和可选接入工具之间的边界。

## 让 KIN 可见

KIN 不是通过夜间主题、靛蓝强调色或三栏框架被识别。它的特征来自整套界面共同遵循的优先级：

- 先呈现用户任务，再提供解释或装饰。
- 只让一个内容或工作区域取得明确的视觉优先级；导航和属性区域用于支持它。
- 优先使用连续结构、对齐和分隔关系，而不是堆叠可互换的卡片。
- 通过删除重复信息形成有效密度，而不是把所有内容缩小。
- 保持结构中性，只为选择、操作和业务含义使用强调。
- 清楚区分事实、状态、可信度、来源和 AI 建议。
- 只用动效解释状态或空间变化，不让静止界面持续吸引注意力。

规范性验收条件和不同产品类型的差异见 [`principles/visual-signature.md`](../principles/visual-signature.md)。
...
| 路径 | 用途 |
|---|---|
| [`VISION.md`](../VISION.md) | 产品方向、范围和发布优先级 |
...
| [`principles/visual-signature.md`](../principles/visual-signature.md) | 可观察的 KIN 视觉签名、产品类型差异与评审门禁 |
| [`principles/motion-vocabulary.md`](../principles/motion-vocabulary.md) | 有目的、可中断、遵循来源和支持减弱动效的操作术语 |
...
| [`scripts/validate-integrations.mjs`](../scripts/validate-integrations.mjs) | 官方运行时包、适配、证据与上游保真检查 |
...
当前目录包含 65 个稳定组件、6 个候选组件和 6 个草稿组件；页面目录包含 10 个稳定页面族、5 个候选页面族和 1 个条件性草稿页面族。KIN 继续采用 contract-first 模式：参考界面和私有 React 实验室用于提供可执行证据，生产接入仍必须通过真实工作流、项目自有决策和已记录的验证来证明。
