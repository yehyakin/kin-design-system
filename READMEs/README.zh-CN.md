...
KIN 2.1 提供不覆盖现有文件的接入记录、验证证据和候选问题审计：
...
初始化器还会创建机器可读的接入证据记录，所有检查初始均为 `not-run`。审计器只把代码模式列为待复核项，不会把正则命中当成视觉结论，也不会自动改写源码。交付边界、证据阶段、例外格式和产品类型见 [`adoption/README.md`](../adoption/README.md)。
...
```md
修改界面前先阅读 DESIGN.md 和 DELIVERY.md。
...
| [`components/workspace-structure.md`](../components/workspace-structure.md) | Location Bar、Toolbar 与 Split View 的结构、输入、响应式和持久化合同 |
...
| [`examples/workspace-reference/core-components.html`](../examples/workspace-reference/core-components.html) | 六组核心组件合同的可运行参考页 |
...
| [`examples/page-patterns/`](../examples/page-patterns/) | 访问、设置流程、偏好、系统恢复、搜索结果与帮助支持的双语参考页 |
