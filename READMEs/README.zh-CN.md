...
Agent Distribution Layer 可以在对应 `main` 部署通过逐字节响应校验后，把已审核的当前合同以原始 Markdown 和 JSON 发布到可变的 [`/next/`](https://yehyakin.github.io/kin-design-system/next/zh/design.md) 通道，覆盖四种主题/对比度模式和两种仓库语言。`next` 通常跟随经过校验的 `main`，不能作为生产固定版本。从暂存发布候选进入 `main` 到最终 GitHub Release 发布之间，完整 Pages 部署会被暂缓；公开展示页与 `/next/` 保持在上一个已验证部署，避免提前出现无效的 Release 链接或不成立的正式发布声明。[`versions.json`](https://yehyakin.github.io/kin-design-system/versions.json) 只在其声明的 Schema 可公开访问后成为发布状态依据；只有对应 KIN 发布通过归档、Tag、GitHub Release 和独立晋升门禁后，才会出现不可变的 `/versions/vX.Y.Z/` 与根稳定别名。目前还没有稳定 Agent 版本，也不会事后补造 v2.3.0 归档。在新的端点通过 `main` Pages 门禁前，请把这些链接视为预留地址。现阶段的生产项目应使用[完整的 v2.3.0 合同](https://github.com/yehyakin/kin-design-system/blob/v2.3.0/DESIGN.md)以及[同一固定版本中的 Skill](https://github.com/yehyakin/kin-design-system/tree/v2.3.0/skills/kin-design)。
...
| [`distribution/`](../distribution/) | 非规范的 Agent 路由、本地化输入、发布 Schema 和版本接口 |
| [`generated/agent/`](../generated/agent/) | 可变 `next`、版本注册表与只创建一次的发布归档 |
