...
已接受的 Agent Distribution Layer 还会在 [`generated/agent/next/`](../generated/agent/next/) 中生成精简、已解析主题的文件，覆盖四种主题/对比度模式和两种仓库语言。但这些仍是可变的开发产物：文案明确标记为未审核，文件尚未发布到 Pages，也没有稳定的 Agent 地址。生产项目在完成后续语言审核和不可变发布门禁前，应使用[完整的 v2.3.0 合同](https://github.com/yehyakin/kin-design-system/blob/v2.3.0/DESIGN.md)以及[同一固定版本中的 Skill](https://github.com/yehyakin/kin-design-system/tree/v2.3.0/skills/kin-design)。
...
| [`distribution/`](../distribution/) | 非规范的 Agent 路由、本地化输入和 Phase 1 Schema |
| [`generated/agent/next/`](../generated/agent/next/) | 已生成的可变开发快照与清单；不是稳定公开地址 |
