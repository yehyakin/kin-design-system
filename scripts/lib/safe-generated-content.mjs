...
function containsRawHtml(value) {
  ...
  // brackets and both HTML comment closing forms without parsing HTML via regex.
...
export function findUnsafeLocaleText(value, label = "locale text") {
  ...
  if (containsRawHtml(value)) findings.push(`${label}: raw HTML is not allowed`);
...
export function scanGeneratedArtifact(name, source) {
  ...
  if (name.endsWith(".md")) {
    if (containsRawHtml(text)) findings.push(`${name}: raw HTML detected`);
