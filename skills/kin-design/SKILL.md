...
2. Read `VISION.md` and `principles/visual-signature.md` when the task changes a page, complete workflow, product composition, showcase, or KIN adoption claim.
...
If `kin.config.json` records `scope.implementationBrief`, read that file before changing a scoped page. Do not implement the representative workflow while the brief is `draft`, contains `TODO`, lacks a route/profile map, or identifies a gallery, Storybook, fixture, or isolated header as production evidence.
...
Do not claim visible KIN adoption only because Tokens, themes, wrappers, or a component gallery exist. A visible-adoption claim also requires a named production workflow and a recorded human visual-signature review.
